...
export async function POST(req: Request) {
  ...
  const created = await prisma.post.create({ data: { workspaceId: ws, channelId: channel.id, title: body.title || "", body: (body as any).body || null, media: (body as any).media || null, status: toPrismaStatus(body.status), scheduledAt: dateIso }, include: { channel: true } });
